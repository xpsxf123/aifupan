...
def main() -> int:
    ...
    parser.add_argument("--path", help="directory root to recurse (legacy mode)")
    parser.add_argument("--changed-paths", default="", help="comma-separated file list (CI/run.py mode); non-.java entries are ignored")
    parser.add_argument("--fail-on-missing", action="store_true", help="Default is WARN; set to FAIL")
    args = parser.parse_args()

    targets: list[str] = []
    if args.changed_paths:
        for raw in args.changed_paths.split(","):
            p = raw.strip()
            if p.endswith(".java") and os.path.isfile(p):
                targets.append(p)
        if not targets:
            print("OK: java comment linter — no .java files in changed-paths, nothing to scan")
            return 0
    elif args.path:
        if not os.path.isdir(args.path):
            print(f"FAIL: path not found: {args.path}")
            return EXIT_FAIL
        targets = _iter_java_files(args.path)
    else:
        print("FAIL: either --path or --changed-paths must be provided")
        return EXIT_FAIL
