...
REQUIRED_PATHS = [
    "CLAUDE.md",
    ".claude/llm_wiki/KNOWLEDGE_GRAPH.md",
...
def check_forbidden_tokens(repo_root: Path) -> List[Finding]:
    ...
    for p in _iter_text_files(repo_root):
        ...
        if rel in IGNORE_FILES:
            continue
        if rel.startswith(".claude/runs/zero_residue_report_"):
            continue
        try:
            lines = _read_lines(p)
        except Exception:
...
def check_markdown_links(repo_root: Path, roots: List[Path]) -> List[Finding]:
    ...
    for f in md_files:
        ...
        if f not in referenced:
            if str(f).startswith(str((repo_root / ".claude" / "llm_wiki").resolve())):
                findings.append(
                    Finding(
                        kind="ORPHAN_MD",
...
def write_report(repo_root: Path, out_path: Path, findings: List[Finding]) -> None:
    ...
    lines.append("- Markdown 死链/孤岛（以 `.claude/llm_wiki` + 入口文档为主）\n")
    lines.append("\n## 禁用关键字\n")
...
def main() -> int:
    ...
    parser.add_argument("--out", default="", help="输出报告路径（默认写入 .claude/runs/）")
    ...
    link_roots = [
        ...
        repo_root / ".claude" / "llm_wiki",
    ]
    findings.extend(check_markdown_links(repo_root, link_roots))

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = Path(args.out) if args.out else (repo_root / ".claude" / "runs" / f"zero_residue_report_{stamp}.md")
