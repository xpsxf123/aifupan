...
**Project override:** in this repo (back-fupan-server), ADRs live at `.claude/llm_wiki/wiki/architecture/adrs/`, NOT `docs/adr/`. The path below is the skill's generic default — substitute the project path when the skill is invoked from this codebase.

```
.claude/llm_wiki/wiki/architecture/adrs/      ← THIS project
├── index.md                                  ← index of all ADRs (analogue of README.md)
├── NNNN-<slug>.md                            ← one file per decision (e.g. 0001-use-rocketmq-for-retry.md)
└── template.md                               ← blank template (optional)
```

(Generic skill default, for other projects):
```
docs/
└── adr/
    ├── README.md
...
**Lifecycle in this project:** ADRs are first drafted **inline in `<run_dir>/openspec.md` §9** by `@system-architect` during Propose phase. At Archive phase, `@architecture-curator` extracts each ADR block, assigns the next NNNN from `wiki/architecture/adrs/index.md`, writes a standalone file, and appends an index row. Architects do NOT directly create files in `wiki/architecture/adrs/` during Propose.
