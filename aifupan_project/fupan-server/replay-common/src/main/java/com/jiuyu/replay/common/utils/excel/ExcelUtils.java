...
public class ExcelUtils {
    ...
    /**
     * 读取Excel内容（默认标准）, 标题在第一行，读取第一个 sheet 且无密码
     *
     ...
     */
    public static <T> void readExcel(InputStream inputStream, Class<T> clazz, RowRead<T> rowRead) {
         readExcel(inputStream, clazz, rowRead, 1, 0, null);
    }
    ...
    public static <T> ExcelWriter buildWrite(Class<T> clazz, OutputStream outputStream, String password, Set<String> excludeColumnFieldNames, ExcelTypeEnum type, String templatePath, Function<ExcelWriterBuilder, ExcelWriter> customized) {
        try {
            ExcelWriterBuilder builder = EasyExcelFactory.write(outputStream, clazz)
                ...
                .registerWriteHandler(new HorizontalCellStyleStrategy(ExcelUtils.getHeadStyle(IndexedColors.WHITE, (short) 14), ExcelUtils.getContentStyle((short) 10)))
                .password(password)
                .excelType(type)
