package com.jiuyu.replay.order.repository.service;

import com.baomidou.mybatisplus.extension.service.IService;
import com.jiuyu.replay.order.entity.TenantPowerRollupEntity;
import com.jiuyu.replay.order.vo.PowerAggregateVo;

import java.util.Date;
import java.util.List;

/**
 * 租户算力消耗汇总表
 *
 * @author lead-engineer
 * @date 2026-07-24
 */
public interface TenantPowerRollupService extends IService<TenantPowerRollupEntity> {

    /**
     * 按 tenant_id 批量累加租户算力消耗（+= delta，非覆盖）
     *
     * @param deltaList  待累加的增量行
     * @param updateDate 刷新时间
     *
     * @return 受影响行数
     */
    int accumulateTenantPowerConsume(List<PowerAggregateVo> deltaList, Date updateDate);
}
