...
@Component
@Scope("prototype")
public class ScriptSentenceImpl extends DefaultSentenceMarkImpl {
    ...
    public String minuteParagraphContent() {
        ...
        boolean startTimeFlag = ((Integer) dataDisplay.getOrDefault("startTime", 0)) == 1; // 开始时间
        boolean natureTimeFlag = ((Integer) dataDisplay.getOrDefault("natureTime", 0)) == 1; // 自然时间-单前无用
        boolean onlineNumFlag = ((Integer) dataDisplay.getOrDefault("onlineNum", 0)) == 1; // 在线人数
        boolean barrageNumFlag = ((Integer) dataDisplay.getOrDefault("barrageNum", 0)) == 1; // 弹幕人数
        boolean dealNumFlag = ((Integer) dataDisplay.getOrDefault("dealNum", 0)) == 1; // 成交数量-单前无用
        ...
        if (ObjectUtil.isNotEmpty(list)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (onlineNumFlag) temp.append(StrUtil.format("\n本段在线人数：{}", renShu));
    ...
    public String getTextParamsContent() {
        ...
        // 获取视频的结束时间
        String str = "";
        if (startTimeFlag || onlineNumFlag) str += StrUtil.format("\n文本内的参数说明：");
        if (startTimeFlag) str += StrUtil.format("\n本段开始时间：指的是转译成的文字段落开始时对应的视频播放时间。");
        if (startTimeFlag) str += StrUtil.format("\n本段结束时间：指的是转译成的文字段落结束时对应的视频播放时间。");
        if (onlineNumFlag) str += StrUtil.format("\n本段在线人数：指的是转译成的文字段落开始时对应的直播间在线人数。");
