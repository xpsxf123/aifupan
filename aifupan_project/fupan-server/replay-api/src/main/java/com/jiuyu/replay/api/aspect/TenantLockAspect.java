...
public class TenantLockAspect {
    ...
    @Around("@annotation(tenantLock)")
    public Object around(ProceedingJoinPoint joinPoint, TenantLock tenantLock) throws Throwable {
        String prefixKey = tenantLock.prefixKey();
        long expireTime = tenantLock.expireTime();
        long waitTime = tenantLock.waitTime();

        // 假设锁的 key 是由方法参数中的 userId 来构成的
        UserCacheVo user = GlobalObject.getLocalUser();
        String lockKey = prefixKey + user.getActiveTenantId();
