...
- Follow `CLAUDE.md` lifecycle gates (spec first for MEDIUM/HIGH, Approval Gate, Archive/WAL).
- Write plans as runtime artifacts under `.claude/runs/task-briefs/<YYYY-MM-DD>_<slug>_plan.md`.
...
3. **Draft** — Writes a self-contained Markdown plan file to `.claude/runs/task-briefs/`. Every step includes a context brief, task list, verification commands, and exit criteria — so a fresh agent can execute any step without reading prior steps.
...
2. Check for existing workflow artifacts under `.claude/runs/task-briefs/`
3. Check for existing ADR/WAL patterns under `.claude/wiki/` when relevant
4. Gather constraints and standards from `CLAUDE.md` and related SSOT documents when needed
...
Write the plan as a Markdown file to `.claude/runs/task-briefs/<YYYY-MM-DD>_<slug>_plan.md` with this structure:
...
2. Present summary to user:
   ...
   Plan saved to: .claude/runs/task-briefs/<YYYY-MM-DD>_<slug>_plan.md
