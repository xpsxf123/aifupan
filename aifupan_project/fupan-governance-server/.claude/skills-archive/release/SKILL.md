---
name: release
description: "Release workflow executor — reads repo release rules, validates pre-release gates (tests, changelog, version bump), then guides step-by-step release execution. TRIGGER when user requests a release, version tag, or deployment."
---
...
A thin, repo-aware release assistant. On first run it inspects the project and CI to derive release rules, stores them under `.claude/runs/task-briefs/` as a runtime artifact, then walks you through a release using those rules.
...
Check whether a cached release rule artifact exists under `.claude/runs/task-briefs/` (e.g., `<YYYY-MM-DD>_<slug>_release_rule.md`).
...
Inspect the repo and answer the following. Write answers into the release rule artifact under `.claude/runs/task-briefs/`.
...
- The release rule artifact under `.claude/runs/task-briefs/` is runtime-only. If you want to persist it, archive it to `.claude/wiki/archive/` as part of the Archive phase.
