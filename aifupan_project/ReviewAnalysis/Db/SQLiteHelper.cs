...
namespace ReviewAnalysis.Db
{
    public class SQLiteHelper
    {
        ...
        private string _sqlFilePath = "script";

        public string SQLiteConnString { get; }

        public SQLiteHelper()
        {
            _dbName = "review_analysis.db";
        ...
        /// 创建数据库文件
        /// </summary>
        /// <param name="initFileName">数据库脚本文件名称</param>
        /// <returns></returns>
        public Boolean createDbFile(string initFileName)
        {
            string dbFile = "";
            try
            {
                if (FileUtils.createDirectory(_dbFilePath))
                {
                    ...
                    if (!FileUtils.isExitsFile(_dbFilePath + "\\" + _dbName, out dbFile))
                    {
                        ...
                        //string sqlScriptPath = dbFile.Replace(_dbName, initFileName);
                        string sqlScriptPath=_sqlFilePath+ "\\" + initFileName;
                        string script = System.IO.File.ReadAllText(sqlScriptPath);
            ...
            catch (Exception ex)
            {
                ...
                FileUtils.log("新建数据库文件" + dbFile + "失败：" + ex.Message);
                return false;
            }
        }
        ...
        public Boolean createDbFile()
        {
            ...
            try
            {
                if (FileUtils.createDirectory(_dbFilePath))
                {
                    ...
                    {
                        SQLiteConnection.CreateFile(dbFile);
                        string sqlScriptPath = dbFile.Replace("review_analysis.db", "init.sql");
                        string script =System.IO.File.ReadAllText(sqlScriptPath);
                        this.initDbTable(script);
                    }
                }
        ...
        public Boolean OpenDb()
        {
            ...
            catch (Exception ex)
            {
                ...
                FileUtils.log("打开数据库：" + _dbName + "的连接失败：" + ex.Message);
                return false;
            }
        }
        ...
        public DataTable ExecuteQuery(string sql, params SQLiteParameter[] parameters)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(this._SQLiteConnString))
            {
                ...
                using (SQLiteCommand command = new SQLiteCommand(sql, connection))
                {
                    ...
                    {
                        FileUtils.log($"错误信息：{ex.Message.ToString()}", "执行一个查询语句，返回一个包含查询结果的DataTable。");
                    }
                    return data;
                }
            }
        ...
        public List<string> GetColumnNames(string tableName)
        {
            ...
            {
                FileUtils.log($"错误信息：{ex.Message.ToString()}", "通过数据表，获取数据表的所有字段");
                return null;
            }
            ...
            using (SQLiteConnection conn = new SQLiteConnection(this._SQLiteConnString))
            {
                using (SQLiteCommand cmd = new SQLiteCommand(conn))
                {
                    try
                    {
                        ...
                        LogServerUtils.LogError("执行查询语句，并返回第一个结果。", "SQLiteHelper.ExecuteScalar", e.Message.ToString());
                        return null;
                    }
                }
            }
            ...
            using (SQLiteConnection connection = new SQLiteConnection(this._SQLiteConnString))
            {
                using (SQLiteCommand command = new SQLiteCommand(connection))
                {
                    try
                    {
                        ...
                        LogServerUtils.LogError("带参数，执行脚本", "SQLiteHelper.ExecuteNonQuery", e.Message.ToString());
                        return 0;
                    }
                }
            }
        ...
        public int ExecuteNonQuery(List<string> a_listSqls)
        {
            ...
            {
                LogServerUtils.LogError("带事务回滚批量执行脚本", "SQLiteHelper.ExecuteNonQuery", e.Message.ToString());
                return 0;
            ...
            using (SQLiteConnection connection = new SQLiteConnection(this._SQLiteConnString))
            {
                ...
                using (SQLiteCommand command = connection.CreateCommand())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        FileUtils.log(ex.Message);
                        LogServerUtils.LogError("执行sql脚本", "SQLiteHelper.initDbTable", ex.Message.ToString());
                    }
                }
            }
        }
