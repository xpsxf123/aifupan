...
class Spider(object):
    ...
    def search_item(self):
        ...
        while True:
            ...
            if json_data:
                ...
                for i in json_data.get('data'):
                    ...
                    # 排除图文数据
                    dynamic_cover_url = jmespath.search('video.dynamic_cover.url_list[0]', aweme_info)
                    if dynamic_cover_url is not None:
                        # 去重
                        if aweme_id not in aweme_id_set:
                            # 集合添加id
                            ...
                            print(item)
                            awemeList.append(item)

                            if len(awemeList) >= 150:
        ...
        print('======== 爬取完成 ==========')
        print(f"共{current_page - 1}页，一共{len(awemeList)}个作品")

        # 检查去重
        print(aweme_id_set)
        print(f'去重之后，一共{len(aweme_id_set)}')
