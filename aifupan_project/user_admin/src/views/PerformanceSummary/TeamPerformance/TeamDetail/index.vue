  ...
  <div class="page-container">
    ...
    <div class="header-info">
      <div class="header-left">
        <el-icon class="company-icon" :size="64"><UserFilled /></el-icon>
    ...
    <div class="tab-container">
      <PageTabs v-model="activeTab" :tabs="tabs">
        ...
        <template #overview>
          <DataOverviewTab :source-id="teamId" source-type="team" :date-range="dateRange" />
        </template>
        <template #liveRoom>
          <LiveRoomDataTab :team-id="teamId" :date-range="dateRange" @view-live-room="handleViewLiveRoom" />
        </template>
      </PageTabs>
    </div>
  </div>
...
<script setup>
  ...
  import { Coin, Money, UserFilled, Promotion, VideoCamera, View, Wallet } from '@element-plus/icons-vue'
  ...
  const getHeaderData = async () => {
    const [orgRes, statsRes] = await Promise.all([
      apiModule.performanceSummary.orgCount({ sourceId: teamId.value, sourceType: 'team' }),
      apiModule.performanceSummary.periodStats({ sourceId: teamId.value, sourceType: 'team' })
    ])
    const { data } = orgRes || {}
    ...
    headerInfo.value = {
      ...headerInfo.value,
      organization: [data.companyName, data.deptName, data.teamName].filter(Boolean).join('-') || '-',
      sourceName: data?.sourceName,
      liveRoomCount: data?.liveRoomCount || 0
    }
