      ...
      >
        ...
        <template #department="{ row }">
          <div class="info-cell">
            ...
            <div class="sub">{{ row.companyName || '-' }}</div>
          </div>
        </template>
...
<script setup>
  ...
  const tableColumns = [
    ...
    { label: '部门', prop: 'name', minWidth: 200, slotName: 'department' },
  ...
  const handleViewDetail = (row) => {
    ...
    router.push({
      ...
      params: { id: row.id },
      query: { name: row.name, tab: 'overview' }
    })
  }
