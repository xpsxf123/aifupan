<template>
  <div class="live-room-performance-page" v-empty="emptyConfig">
    <Curd
      ref="curdRef"
    ...
    >
      ...
      <template #date>
        <div class="header-filters">
          ...
          <ExportDataButton class="export-btn" :data="exportList" :columns="tableColumns" file-name="直播间业绩列表" />
        </div>
      </template>
      ...
      <!-- Live Room Info Column -->
      <template #liveRoom="{ row }">
        <LiveRoomInfo :row="row" />
      </template>
    </Curd>
  </div>
</template>
...
<script setup>
  ...
  import LiveRoomInfo from './components/LiveRoomInfo.vue'
  ...
  // Custom layout: Search on left, Filters on right in the header
  ...
  const api = {
    list: async (params) => {
      ...
      const res = await apiModule.performanceSummary.liveRoomPage({
        name,
        startDate,
        endDate,
        page: page || 1,
  ...
  const handleViewDetail = (row) => {
    ...
    router.push(`/live-room-performance/detail/${row.id}`)
  }
