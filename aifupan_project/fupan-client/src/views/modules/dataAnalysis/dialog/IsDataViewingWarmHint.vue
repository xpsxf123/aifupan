<template>
    <WarmHint ref="warmHint" @left-click="warmLeftClick" @right-click="warmRightClick"  :rightBt="getRightBt" leftBt="知道了">
       <div v-if="isActivate || isFree">
            您当前版本为<span class="text-colorTheme">{{getVersionName}}</span>，数据看板直播间数量<span class="text-colorErr">暂无授权</span>，请升级到高版本后再进行操作
       </div>
       <div v-else > 
            您的数据看板数量<span  class="text-colorErr">授权不足</span>，请关闭其他直播间数据看板，或<span class="text-colorTheme">立即扩容</span>
       </div>
    </WarmHint>
</template>
