...
// 获取当前日期
export var getDate = ({ stamp = "", symbol = "-" }) => {
...
// 中文正则
export var reg_cn = (val) => {
  let reg = new RegExp("[\u4E00-\u9FA5]+");
  return reg.test(val);
};
// 英文正则
export var reg_en = (val) => {
 ...
 * 清除登录信息
 */
export function clearLoginInfo(type) {
  store.commit("saveLoginResultData");
  if(type === 'user'){return}
  store.commit("setLoadRouterFlag", false);
}