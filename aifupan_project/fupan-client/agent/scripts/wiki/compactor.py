...
Tool to merge fragmented markdown files in agnet/wiki/*/wal into the corresponding domain's index.md.
"""
...
def _merge_wal_dir(wal_dir):
    ...
    if current_lines >= REFACTOR_THRESHOLD_LINES:
        ...
        print(f"    Compaction aborted for domain '{domain_name}'. Please refactor and split the index.")
        return [], True
...
def main():
    ...
    if needs_refactor:
        print("\n❌ Compaction halted due to capacity limits.")
        sys.exit(2)
